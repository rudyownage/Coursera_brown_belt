...
public:
    explicit HashSet(size_t num_buckets, const Hasher& hasher = {}) : num_b(num_buckets), hasher_(hasher), data(num_buckets)
    {
    }

    void Add(const Type& value) {
        if (!Has(value)) {
            data[GetBucketIndex(value)].push_front(value);
        }
    }
    ...
    void Erase(const Type& value) {
         data[GetBucketIndex(value)].remove(value);       
    }
